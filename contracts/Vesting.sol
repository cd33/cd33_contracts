...
// use of safeBatchTransferFrom to save gas costs
// optimisation with external not public
// use of Pausable
// Removal of initStaking, useless thanks to Pausable
// Replacement of AccessControl with Ownable, more economical
// Prettier for indentation

// !!!!!!!!!!!!!!!! NOTER LES MODIFS POUR LE PULL REQUEST !!!!!!!!!!!!!!!!!!!
...
contract Vesting is Ownable, ERC721Holder, ReentrancyGuard, Pausable {
    ...
    function GetStakedERC721(address _user, uint256 _tokenId) view external returns(ItemInfo memory) {
        return stakers[_user].token[_tokenId];
    }

    function GetStakedERC1155(address _user, uint256 _tokenId) view external returns(ItemInfo[]  memory) {
        return stakers[_user].multiToken[_tokenId];
    }

    function GetAllStakedERC721(address _user) external view returns (uint256[] memory tokenIds)
    {
        return stakers[_user].ownedTokens;
    }

    function GetAllStakedERC1155(address _user) external view returns (uint256[] memory tokenIds)
    {
    ...
    function stakeERC1155(uint256 tokenId, uint16 tokenAmount) external whenNotPaused {
    ...
    function batchStakeERC721(uint256[] memory tokenIds) external whenNotPaused {
    ...
    function batchStakeERC1155(uint256[] memory tokenIds, uint256[] memory tokenAmounts) external whenNotPaused {
        require(
    ...
    function batchUnstakeERC1155(uint256[] memory tokenIds, uint256[] memory tokenIndexes) external{
        require(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(tokenIndexes[i] < stakers[msg.sender].multiToken[tokenIds[i]].length ,"Nft Staking System: out of bounds token index");

            elapsedTime = block.timestamp - uint(stakers[msg.sender].multiToken[tokenIds[i]][tokenIndexes[i]].timestamp);
            require(stakingUnlock < elapsedTime, "Staked token cannot be unstaked as minimum period has not elapsed");
        }
    ...
    function _batchUnstakeERC1155(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if(_elapsedTime > stakingMinimum && tokensClaimable == true){ 
                stakedPeriods = (block.timestamp - item.timestamp) / stakingPeriod;
